...
<!-- FSCK:START -->
> [!WARNING]
> **filesystem check required.** 10 corrupted sectors detected on `/dev/sda1` (board #1, 0/71 verified). any operator may run `fsck` — click a sector below to scan it. scanning a corrupted sector panics the kernel.
...
<sub>disk encryption: none. the corruption map is technically derivable from source. the daemon believes in open source and in honor systems.</sub>

**S.M.A.R.T. log** — no operations recorded. the disk sits in silence.
